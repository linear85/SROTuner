from SROTuner.tuner.Alloy import Alloy
from SROTuner.tuner.SRO_Data import SRO_Data
import random
import numpy as np
...
class Ternary(Alloy):
    ...
    def create_SRO_Data(self):
        return SRO_Data(self.readFile, self.N, 3)
    
    def getWantWCPs(self, WCPs:np) -> np:
        compArray = np.array([[self.comp[0], self.comp[1], self.comp[2]],
                              [self.comp[0], self.comp[1], self.comp[2]],
                              [self.comp[0], self.comp[1], self.comp[2]]]) * self.N
